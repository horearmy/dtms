...
export async function GET(_req: NextRequest) {
  ...
  return runWithTenant(null, async () => {
    ...
    const [
      ...
      prisma.auditLog.count({ where: { createdAt: { gte: new Date(now - hour) }, action: { contains: 'FAILED', not: { contains: 'LOGIN' } } } }),
      prisma.auditLog.count({ where: { createdAt: { gte: new Date(now - hour * 2), lt: new Date(now - hour) } } }),
      prisma.auditLog.count({ where: { createdAt: { gte: new Date(now - hour * 2), lt: new Date(now - hour) }, action: { contains: 'FAILED', not: { contains: 'LOGIN' } } } }),
